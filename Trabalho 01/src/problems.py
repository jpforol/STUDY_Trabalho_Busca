...
class MazeProblem(ProblemInterface):
    def __init__(self, n_rows: int, n_cols: int, seed: Optional[int] = 6749053793017053058):
        if seed is not None:
            random.seed(seed)
    ...
    def actions(self, state: Tuple[int, int]) -> List[Tuple[int, int]]:
        ...
        neighbors_coordinates = [
            [state[0]+0, state[1]+1], # Direita
            [state[0]+1, state[1]+1], # Diagonal Direita Baixo
            [state[0]+1, state[1]+0], # Baixo
            [state[0]-1, state[1]+0], # Cima
            [state[0]+1, state[1]-1], # Diagonal Esquerda Baixo
            [state[0]-1, state[1]+1], # Diagonal Direita Cima
            [state[0]+0, state[1]-1], # Esquerda
            [state[0]-1, state[1]-1], # Diagonal Esquerda Cima    
